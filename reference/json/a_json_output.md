...
Each Spoon command-line interface command can be run with `--format=json` switch to get output in JSON format. It is useful when running Spoon in a scripted environment. When using this switch full output is available when command ends - no progress and partial results are presented during execution, even for commands that take a long time.

### format

Output format is an array of JSON object. For example, here is result of executing `spoon vm --format=json`:
...
Each object represents result of one command. Currently `spoon` allows to execute one command at a time so it contains one JSON object.
...
* `messages` - array of strings that would be output by `spoon` if `--format=json` parameter was not passed
