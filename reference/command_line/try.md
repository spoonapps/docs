...
> spoon try --network=host <image>

# Launch two containers in a "mynet" virtual network
> spoon try -d --network=mynet --name=web <image>
web

> spoon try -d --network=mynet myself/webbrowser http://web
...
```
spoon try --mount "C:\FolderOnHostSystem=C:\FolderInContainer" clean
```

Mounts are useful to share a cache folder, like a local Maven repository:
```
spoon try --mount "%USERPROFILE%\.m2=%USERPROFILE%\.m2" jdk,maven
```

Mounting multiple folder is done by repeating the mount parameter:

```
spoon try --mount "C:\Mount1=C:\InContainer1" --mount "C:\Mount2=C:\InContainer2" clean
```

It is also possible to mount a folder from another container:

```
spoon try --mount <containerid>:"C:\FolderInSourceContainer=C:\FolderInTargetContainer" clean
```
